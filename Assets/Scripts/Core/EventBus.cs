...
public class EventBus
{
    ...
    static readonly Dictionary<Type, IList> _topics = new();
    ...
    public static void Publish<T>(T published_event)
    {
        ...
        if (_topics.ContainsKey(t))
        {
            ...
            List<Subscription<T>> orphaned_subscriptions = new();
        ...
        {
                if (DEBUG_MODE) Debug.Log("...but no one is subscribed to this event right now.");
        }
    }
    ...
    public static Subscription<T> Subscribe<T>(Action<T> callback)
    {
        ...
        Type t = typeof(T);
        Subscription<T> new_subscription = new(callback);
